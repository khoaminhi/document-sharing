...
if (!function_exists('create_MongoId')) {

    function create_MongoId($id) {
...
Class Mongo_db
{
     ...
     *  2) Low priority - values specified as default for class parameters
     */
    private $config = [
        'settings'          => [],
        'connection_string' => '',
    ...
    /**
     * 
     * @param type $mongodbname
     * @param type $mongohost
     * @param type $mongoport
     * @param type $options
     */
    public function __construct($mongodbname = 'training', $mongohost = '127.0.0.1', $mongoport = 27017, $options = array()) {
        $connection = ['host' => $mongohost, 'port' => $mongoport . '', 'db_name' => $mongodbname, 'db_options' => $options];
        ...
        if (count($connection) > 0) {
            $this->config['connection'] = array_replace_recursive($this->config['connection'], $connection);
        }
    ...
    public function count2(string $collection = '') {
        if (empty($collection)) {
        ...
        try {
            ...
            if (((int) $this->limit) < 999999) {
                $command['limit'] = (int) $this->limit;
            }
            if (((int) $this->offset) > 0) {
                $command['skip'] = (int) $this->offset;
            }
            
            $read_concern = new MongoDB\Driver\ReadConcern($this->read_concern);
            $command['readConcern'] = $read_concern;
            
            $resultCount = $this->command($command);
            if (isset($resultCount[0]["n"])) {
                return $resultCount[0]["n"];
            }
            else if (isset($resultCount["n"])){
    ...
    public function countDocuments(string $collection = '', array $options = [])
    {
        if(empty($this->wheres)){
            $pipeline = [
     ...
     *  @return  $this
     */
    private function config(array $config = []): self {
        if (count($config) > 0) {
            $this->config = $config;
        }
